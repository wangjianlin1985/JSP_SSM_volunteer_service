...
public class SignUpController extends BaseController {
	...
	/*前台志愿者客户端ajax方式提交添加项目报名信息*/
	@RequestMapping(value = "/userAdd", method = RequestMethod.POST)
	public void userAdd(SignUp signUp, BindingResult br,
		...
		if(signUpService.querySignUp(signUp.getActivityObj(), userObj, "").size() > 0) {
			message = "这个项目你已经报过名了！";
			writeJsonResponse(response, success, message);
			return ;
		}
	...
	public String userFrontlist(@ModelAttribute("activityObj") ActivityInfo activityObj,@ModelAttribute("userObj") UserInfo userObj,String signUpTime,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (signUpTime == null) signUpTime = "";
	    ...
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
		return "SignUp/signUp_userFrontquery_result"; 
	}
	...
	public void OutToExcel(@ModelAttribute("activityObj") ActivityInfo activityObj,@ModelAttribute("userObj") UserInfo userObj,String signUpTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "报名id","报名的项目","报名人","报名宣誓","报名时间"};
